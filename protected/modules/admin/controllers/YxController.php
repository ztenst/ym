 ...
 * @date(2017.2.5)
 */
class YxController extends AdminController{
	...
	public function actionList($type='title',$value='',$time_type='created',$time='',$cate='')
	{
		...
		$news = YxExt::model()->getList($criteria,20);
		// 这个是渲染页面
		$this->render('list',[
			// 'cates'=>$this->cates,
			'news'=>$news->data,
	...
	{
		$info = $id ? YxExt::model()->findByPk($id) : new YxExt;
		if(Yii::app()->request->getIsPostRequest()) {
			$info->attributes = Yii::app()->request->getPost('YxExt',[]);
			// $info->mid = $mid
			if($info->save()) {
